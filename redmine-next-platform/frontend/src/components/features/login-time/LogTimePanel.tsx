...
export function LogTimePanel({ reportDate, userName }: Props) {
        ...
        {state.kind === "ready" && data && (
          <div className="space-y-4">
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10">
              <div className="hidden grid-cols-[1fr_2fr_0.8fr_0.6fr] gap-4 border-b border-white/10 bg-black/30 px-4 py-3 text-xs uppercase tracking-[0.2em] text-slate-400 md:grid">
              ...
              <div className="divide-y divide-white/10">
                {data.items.map((item) => (
                  <div key={item.issueKey} className="grid gap-2 px-4 py-3 text-sm transition hover:bg-white/[0.04] md:grid-cols-[1fr_2fr_0.8fr_0.6fr] md:items-center">
