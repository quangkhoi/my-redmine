...
public sealed class MyTaskController : ControllerBase
{
    ...
    public async Task<IActionResult> Get([FromRoute] string userName, [FromServices] GetMyTaskHandler handler, CancellationToken cancellationToken)
    {
        var result = await handler.Handle(new GetMyTaskQuery(userName), cancellationToken);
